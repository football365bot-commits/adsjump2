...
const bulletPool = Array.from({ length: CONFIG.BULLET_POOL_SIZE }, () => ({
    active: false, x: 0, y: 0, vx: 0, vy: 0, speed: CONFIG.BULLET_SPEED,
    damage: CONFIG.PLAYER_BULLET_DAMAGE, owner: null
}));
...
class Player {
    ...
    update(inputX) {
        ...
        this.anim.tilt += (inputX * 0.25 - this.anim.tilt) * 0.15;
        this.anim.jump = this.vy < 0 ? 1 : Math.max(0, this.anim.jump - 0.08);
        this.anim.land = this.vy > 0 && this.lastY + this.size <= this.y ? 1 : Math.max(0, this.anim.land - 0.12);
...
// =====================
class Enemy {
    constructor() { this.shootCooldown = 0; this.reset(); }
    ...
    spawn(x, y, type) {
        this.reset(); this.x = x; this.y = this.baseY = y; this.type = type; this.active = true;
    ...
    draw(cameraY){ if(!this.active)return;
        ctx.save();
        ...
        ctx.fillStyle='#555';
        ctx.fillRect(-CONFIG.ENEMY_SIZE/2,-CONFIG.ENEMY_SIZE/2-6,CONFIG.ENEMY_SIZE,4);
        ctx.fillStyle='#0f0';
        ctx.fillRect(-CONFIG.ENEMY_SIZE/2,-CONFIG.ENEMY_SIZE/2-6,CONFIG.ENEMY_SIZE*(this.hp/CONFIG.ENEMY_HP),4);
        ctx.restore();
    }
}
...
class Platform {
    ...
    reset(){ this.x=this.y=this.prevY=this.baseY=0; this.movementType='static'; this.isBroken=false; this.vx=this.vy=this.amplitude=0; this.active=false; this.used=false; }
    spawn(x,y,movementType,isBroken){ this.reset(); this.x=this.baseY=this.prevY=y; this.y=y; this.movementType=movementType; this.isBroken=isBroken; this.active=true;
        ...
        const newItem=getItemFromPool(); if(newItem)newItem.spawn(this);
    }
    update(){ if(!this.active)return; this.prevY=this.y;
        if(this.movementType==='horizontal'){this.x+=this.vx;if(this.x<0||this.x+CONFIG.PLATFORM_WIDTH>canvas.width)this.vx*=-1;}
    ...
    draw(cameraY){ if(!this.active)return;
        ctx.fillStyle = this.isBroken?'#f44':(this.movementType==='vertical'?'#88f':(this.movementType==='horizontal'?'#0ff':'#0f8'));
        ctx.fillRect(this.x,this.y-cameraY,CONFIG.PLATFORM_WIDTH,CONFIG.PLATFORM_HEIGHT);
    }
    checkCollision(player){ if(!this.active)return false;
        const prevBottom=player.lastY+player.size;
        const currBottom=player.y+player.size;
        if(player.vy>0 && prevBottom<=this.prevY+CONFIG.PLATFORM_HEIGHT && currBottom>=this.prevY && player.x+player.size>this.x && player.x<this.x+CONFIG.PLATFORM_WIDTH){
...
spawn(platform){ if(!platform)return; const r=Math.random(); 
if(r<0.0003)this.type='rocket'; else if(r<0.0007)this.type='drone'; else if(r<0.0014)this.type='trampoline'; else if(r<0.0018)this.type='bomb'; else if(r<0.0025)this.type='spikes'; else if(r<0.0040)this.type='adrenaline'; else if(r<0.0070)this.type='medkit'; else return;
this.active=true; this.platform=platform; this.x=platform.x+CONFIG.PLATFORM_WIDTH/2-this.size/2; this.y=platform.y-this.size; }
update(){ if(!this.active)return; if(!this.platform||!this.platform.active){ this.active=false; this.platform=null; return; }
this.x=this.platform.x+CONFIG.PLATFORM_WIDTH/2-this.size/2; this.y=this.platform.y-this.size;
if(player.x+CONFIG.PLAYER_SIZE>this.x && player.x<this.x+this.size && player.y+CONFIG.PLAYER_SIZE>this.y && player.y<this.y+this.size){
switch(this.type){ case 'trampoline': player.vy-=5; break; case 'drone': player.vy-=35; break; case 'rocket': player.vy-=75; break; case 'spikes': player.hp-=1; break; case 'bomb': player.hp-=5; break; case 'medkit': player.hp=Math.min(player.hp+1,100); break; case 'adrenaline': player.hp=Math.min(player.hp+5,100); break;}
this.active=false; this.platform=null;
}}
draw(){ if(!this.active)return; let color='#fff'; switch(this.type){ case 'trampoline': color='#ff0'; break; case 'drone': color='#f80'; break; case 'rocket': color='#f00'; break; case 'spikes': color='#888'; break; case 'bomb': color='#000'; break; case 'medkit': color='#0f0'; break; case 'adrenaline': color='#f0f'; break;}
ctx.fillStyle=color; ctx.fillRect(this.x,this.y-cameraY,this.size,this.size);}
}
...
update(){ if(!this.active)return; [...enemies,player].forEach(obj=>{ if(obj!==player && !obj.active)return;
const objSize=obj.size||CONFIG.ENEMY_SIZE; const dx=this.x-(obj.x+objSize/2); const dy=this.y-(obj.y+objSize/2); const dist=Math.hypot(dx,dy);
if(dist<this.radius){ const strength=this.pullStrength*(1+(this.radius-dist)/this.radius*4); const angle=Math.atan2(dy,dx)+0.1;
const pullX=Math.cos(angle)*strength; const pullY=Math.sin(angle)*strength;
obj.x+=pullX; obj.y+=pullY; obj.visualScale=Math.max(0.5,dist/this.radius);
if(dist<5)obj.active=false;
if(obj===player && dist<10)gameState=GameState.GAME_OVER;}else obj.visualScale=1;}); }
...
let lastBlackHoleScore=0;
function getBlackHoleFromPool(){ return blackHolePool.find(bh=>!bh.active) || null; }

// =====================
// SCORE
// =====================
const ScoreManager={value:0,maxY:null,
...
function draw(){
    ...
    ctx.textAlign='right';
    ctx.fillText(`${Math.floor(ScoreManager.value)}`,cx-10,30);
    ctx.textAlign='left';
    ctx.fillText(`HP: ${player.hp}`,cx+10,30);
...
function loop(){
    ...
    else if(gameState===GameState.GAME_OVER){ draw(); pauseUI.draw(gameState,coins); }
