...
class Platform {
    ...
    spawn(x, y, movementType, isBroken) {
        this.reset();
        this.x = this.y = this.prevY = this.baseY = y;
...
class Bullet {
    ...
    draw(cameraY) {
        ...
        ctx.fillRect(this.x, this.y - cameraY, CONFIG.BULLET_SIZE, CONFIG.BULLET_SIZE);
    }
}
...
class Enemy {
    ...
    reset() {
        this.x = this.y = this.vx = this.vy = this.baseY = this.amplitude = 0;
        this.type = 'static';
        this.active = false;
        this.hp = CONFIG.ENEMY_HP;
        this.shootCooldown = rand(60,120);
    }

    spawn(x, y, type) {
        this.reset();
        this.x = x; this.y = this.baseY = y;
        this.type = type;
        this.active = true;
        this.hp = CONFIG.ENEMY_HP;
    }
    ...
    update() {
        ...
        if (this.type === 'horizontal') {
            this.x += this.vx || rand(1,2) * (Math.random()<0.5?-1:1);
            if (this.x < 0 || this.x + CONFIG.ENEMY_SIZE > canvas.width) this.vx*=-1;
        }
        if (this.type === 'vertical') {
            this.y += this.vy || rand(1,2);
            if (this.y > this.baseY + this.amplitude || this.y < this.baseY - this.amplitude) this.vy*=-1;
        }
...
function spawnEntities(isReset=false){
    ...
    if(isReset){
        ...
        start.spawn(canvas.width/2-CONFIG.PLATFORM_WIDTH/2, canvas.height-50,'static',false);
        player.y = start.y - player.size;
        maxPlatformY = start.y;
    }
...
function playerAutoShoot(){
    ...
    if(!target) return; // нет врагов — не стреляем
    const bullet = bulletPool.find(b=>!b.active);
    if(!bullet) return;
...
function update(){
    ...
    if(shootTimer-- <=0){
        playerAutoShoot();
        shootTimer = 15; // автострельба каждые 15 кадров
    }
    ...
    bulletPool.forEach(b=>{
        ...
        if(b.fromPlayer){
            enemies.forEach(e=>{
                if(e.active &&
                    b.x < e.x + CONFIG.ENEMY_SIZE && b.x + CONFIG.BULLET_SIZE > e.x &&
            ...
            if(player.hp>0 &&
                b.x < player.x + player.size && b.x + CONFIG.BULLET_SIZE > player.x &&
