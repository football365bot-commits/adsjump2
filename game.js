...
function isOnScreen(obj) {
    return (
        obj.y - cameraY + (obj.size || CONFIG.ENEMY_SIZE) > 0 &&
...
class Player {
    ...
    update(inputX) {
        ...
        if (this.vy > 0 && this.lastY + this.size <= this.y) {
            this.anim.land = 1;
        }
        ...
        if (this.shootCooldown <= 0) {
            ...
            if (target && isOnScreen(this)) {
                ...
                const dx =
                    target.x + CONFIG.ENEMY_SIZE / 2 -
                    (this.x + this.size / 2);

                const dy =
                    target.y + CONFIG.ENEMY_SIZE / 2 -
                    (this.y + this.size / 2);
    ...
    draw(cameraY) {
        ...
        const jumpStretch =
            Math.sin(this.anim.jump * Math.PI) * 0.25;

        const landSquash =
            Math.sin(this.anim.land * Math.PI) * 0.2;
...
class Enemy {
    ...
    spawn(x, y, type) {
        ...
        if (type === 'horizontal') {
            this.vx = (rand(1, 2) + 2 * f) * (Math.random() < 0.5 ? -1 : 1);
        }
    ...
    update() {
        ...
        if (this.type === 'horizontal') {
            ...
            if (
                this.x < 0 ||
                this.x + CONFIG.ENEMY_SIZE > canvas.width
            ) {
                this.vx *= -1;
            }
        }
    ...
    draw(cameraY) {
        ...
        ctx.translate(
            ...
            this.y - cameraY + CONFIG.ENEMY_SIZE / 2
        );
        ...
        ctx.fillRect(
            -barWidth / 2,
            -CONFIG.ENEMY_SIZE / 2 - 6,
            barWidth * (this.hp / CONFIG.ENEMY_HP),
            barHeight
        );
...
class Platform {
    ...
    update() {
        ...
        if (this.movementType === 'horizontal') {
            ...
            if (
                this.x < 0 ||
                this.x + CONFIG.PLATFORM_WIDTH > canvas.width
            ) {
                this.vx *= -1;
            }
        }
...
class Item {
    ...
    spawn(platform) {
        ...
        else if (r < 0.0040) this.type = 'adrenaline';
        else if (r < 0.0070) this.type = 'medkit';
        else return;
...
class BlackHole {
    ...
    update() {
        ...
        objects.forEach(obj => {
            ...
            const size = obj.size || CONFIG.ENEMY_SIZE;
            const dx = this.x - (obj.x + size / 2);
            const dy = this.y - (obj.y + size / 2);
            const dist = Math.hypot(dx, dy);

            if (dist < this.radius) {
                const force =
                    this.pullStrength *
                    (1 + (this.radius - dist) / this.radius * 4);
...
const ShootingSystem = {
    ...
    processShots() {
        this.requests.forEach(req => {
            ...
            const dx =
                (req.target.x + CONFIG.ENEMY_SIZE / 2) -
                (req.shooter.x + CONFIG.PLAYER_SIZE / 2);

            const dy =
                (req.target.y + CONFIG.ENEMY_SIZE / 2) -
                (req.shooter.y + CONFIG.PLAYER_SIZE / 2);
...
function spawnEntities(isReset = false) {
    ...
    platforms.forEach(p => {
        if (!p.active) {
            ...
            const minGap = Math.min(CONFIG.MIN_GAP * growth, 95);
            const maxGap = Math.min(CONFIG.MAX_GAP * growth, 110);
